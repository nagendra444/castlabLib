 ...
 * @since 27.04.14.
 */
public class mmHTTPRetryManager {
    /** seconds to wait until next retry */
    protected static final int[] BACKOFF = { 1, 2, 4, 7, 12, 20, 30, 60, 120 };

    static final mmHTTPRetryManager DEFAULT = new mmHTTPRetryManager();
     ...
     */
    public boolean isRetryUseful(mmHTTPResponse response) {
     ...
     * @param mmHTTPException the exception to analyze
     * @return <tt>true</tt> if it makes sense for the request to be retried again.
     */
    public boolean isRecoverable(mmHTTPException mmHTTPException) {
        Throwable cause = mmHTTPException.getCause();
        if (cause == null) {
            return false;
        }
